...
import "./IIPClaimsLicenseTest.sol";
import "../../ECRecover/ECTools.sol";
import "./../../IPClaim/IPClaim.sol";
import "../../Oracle/ETHExchangeOracle.sol";


contract IPClaimsLicenseTest is IIPClaimsLicenseTest, OwnableUpgradableImplementation {
    ...
    modifier onlyNotActiveLicense(address _ipClaimAddress) {
        require(claimToLicenseeToEndDate[_ipClaimAddress][msg.sender] <= now, "There is an active license");
        _;
    }
    ...
    function setCommissionPercentage(uint8 _commissionPercentage) public onlyOwner {
        commissionPercentage = _commissionPercentage;
    }
	...
	*IPClaim address, price in ethers, days/s
    ...
    onlyWithinValidPeriod(_days) {
        ...
        uint256 platformPercentFeeInWei = (_licensePriceInWei.mul(commissionPercentage)).div(100);
    ...
    function extendLicense(address _ipClaimAddress, uint256 _days) internal {
        ...
        if (claimToLicenseeToEndDate[_ipClaimAddress][msg.sender] == 0 || now > claimToLicenseeToEndDate[_ipClaimAddress][msg.sender]) {
            licenseEndDate = (_days.mul(1 days)).add(now);
            claimToLicenseeToEndDate[_ipClaimAddress][msg.sender] = licenseEndDate;
            licenseesPerClaim[_ipClaimAddress].push(msg.sender);
            licensesPerLicensee[msg.sender].push(_ipClaimAddress);
        } else {
            licenseEndDate = (_days.mul(1 days)).add(claimToLicenseeToEndDate[_ipClaimAddress][msg.sender]);
            claimToLicenseeToEndDate[_ipClaimAddress][msg.sender] = licenseEndDate;
        }
    }
